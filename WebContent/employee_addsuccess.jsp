	...
	<pre><b>員工資料維護區</b>									在職人數:<a href="javascript:document.showHire.submit();"><b class="red">${count.total}</b></a>  無E-mail人數:<a href="javascript:document.showNoEmail.submit();"><b class="red">${count.noEmail}</b></a>  無分機人數:<a href="javascript:document.showNoExtension.submit();"><b class="red">${count.noExtension}</b></a>	</pre>
		...
		<table border="1">
			<tbody>
				<tr>
					...
					<c:forEach var="excelEmployee" items="${employee}">
						<tr>
							<td><%=++x%></td>

							<td><input type="button" class="mdbutton" value="修改"
								onclick="modify('${excelEmployee.ID}');"></td>
															
							<td><input type="button" class="mdbutton" value="刪除" onclick="deleteEmployee('${excelEmployee.ID}');"></td>
							
							<td>${excelEmployee.ID}</td>
							<td>${excelEmployee.name}</td>
							<td>${excelEmployee.deployment}</td>
							<td>${excelEmployee.position}</td>
							<td>${excelEmployee.email}</td>
							<td>${excelEmployee.extension}</td>
							<td>${excelEmployee.cardNumber}</td>
							<td>${excelEmployee.idCard}</td>
							<td>${excelEmployee.hireDate}</td>
							<td>${excelEmployee.quitDate}</td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
		...
		<form action="ExportExcel.jsp" method="post">
			<c:if test="${not empty employee}">
				<c:set var="excel" value="${employee}" scope="application"/>
			</c:if>
