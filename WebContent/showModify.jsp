...
<link rel="stylesheet" type="text/css" href="css/setting.css"/>
	...
	<script type="text/javascript">
		function modify(id){ //鎖定子視窗 必須先關閉才能操作母視窗
						
			var iWidth = 1650;  //視窗的寬度;
			var iHeight = 700; //視窗的高度;
			var iTop = (window.screen.availHeight-30-iHeight)/2;  //視窗的垂直位置;
			var iLeft = (window.screen.availWidth-10-iWidth)/2;   //視窗的水平位置;
			window.open('modify.jsp?id='+id,'modify','height='+iHeight+',innerHeight='+iHeight+',width='+iWidth+',innerWidth='+iWidth+',top='+iTop+',left='+iLeft+',resizable=no,scrollbars=no,status=no,depended=yes');			
 ...
 <div class="div1">
	...
	<pre><b>員工資料維護區</b>									在職人數:<a href="javascript:document.showHire.submit();"><b class="red">${count.total}</b></a>  無E-mail人數:<a href="javascript:document.showNoEmail.submit();"><b class="red">${count.noEmail}</b></a>  無分機人數:<a href="javascript:document.showNoExtension.submit();"><b class="red">${count.noExtension}</b></a>	</pre>
		...
		<table border="1">
			<tbody>
				...
				<c:if test="${not empty Modify}">
					<c:forEach var="queryemployee" items="${ModifyList}">
						<tr>
				...
				<c:if test="${not empty Add}">	
					<c:forEach var="addemployee" items="${Add}">
						<tr>
							<td><%=++x%></td>

							<td><input type="button" class="mdbutton" value="修改"
								onclick="modify('${addemployee.ID}');"></td>

							<td><input type="button" class="mdbutton" value="刪除" onclick="deleteEmployee('${addemployee.ID}');"></td>
							
							<td>${addemployee.ID}</td>
							<td>${addemployee.name}</td>
							<td>${addemployee.deployment}</td>
							<td>${addemployee.position}</td>
							<td>${addemployee.email}</td>
							<td>${addemployee.extension}</td>
							<td>${addemployee.cardNumber}</td>
							<td>${addemployee.idCard}</td>
							<td>${addemployee.hireDate}</td>
							<td>${addemployee.quitDate}</td>
							<td>${addemployee.name}</td>
							<td><%=time%></td>
							<td></td>
							<td></td>
				...
				<c:if test="${not empty All}">	
					<c:forEach var="allEmployee" items="${All}">
						<tr>
							<td><%=++x%></td>

							<td><input type="button" class="mdbutton" value="修改"
								onclick="modify('${allEmployee.ID}');"></td>

							<td><input type="button" class="mdbutton" value="刪除" onclick="deleteEmployee('${allEmployee.ID}');"></td>
							
							<td>${allEmployee.ID}</td>
							<td>${allEmployee.name}</td>
							<td>${allEmployee.deployment}</td>
							<td>${allEmployee.position}</td>
							<td>${allEmployee.email}</td>
							<td>${allEmployee.extension}</td>
							<td>${allEmployee.cardNumber}</td>
							<td>${allEmployee.idCard}</td>
							<td>${allEmployee.hireDate}</td>
							<td>${allEmployee.quitDate}</td>
							<td></td>
							<td></td>
							<td></td>
							<td></td>
		...
		<form action="ExportExcel.jsp" method="post">
			...
			<c:if test="${not empty Add}">
				<c:set var="excel" value="${Add}" scope="application"/>
			</c:if>
			<c:if test="${not empty All}">
				<c:set var="excel" value="${All}" scope="application"/>
			</c:if>		
